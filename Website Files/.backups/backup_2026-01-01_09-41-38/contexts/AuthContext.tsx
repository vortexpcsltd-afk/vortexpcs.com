        ...
        import("../services/auth").then(({ getUserProfile }) => {
          getUserProfile(firebaseUser.uid)
            .then(async (profile) => {
              ...
              if (
                firebaseDisplayName &&
                (!profileDisplayName || profileDisplayName === emailPrefix)
              ) {
                profile.displayName = firebaseDisplayName;
                try {
                  ...
                  logger.info(
                    "Synchronized displayName from Firebase profile",
                    {
                      uid: firebaseUser.uid,
                    }
                  );
                } catch (syncErr) {
                  logger.warn("Failed to sync displayName for user", {
                    err: String(syncErr),
                  });
                }
              }
