...
export interface PCComponent {
  ...
  images?: string[];
  imagesByOption?: Record<string, Record<string, string[]>>; // Images organised by option (e.g., { colour: { Black: [...], White: [...] } })
  ...
  try {
    ...
    const products = response.items.map((entry) => {
      ...
      return {
        ...
        reducedPrice: getNumber(f["reducedPrice"]) ?? undefined,
...
function mapContentfulToComponent(
  ...
  return {
    ...
    price: getNumber(fields.price) ?? 0,
    reducedPrice: getNumber(fields.reducedPrice) ?? undefined,
    category: category,
...
> => {
  // bump cache key to refresh after content type rename discovery
  const cacheKey = "gamingLaptops_v2_all";
  const cached =
  ...
  try {
    ...
    // Try multiple content type names in case of naming differences (Contentful ID provided: laptops)
    const possibleTypes = ["laptops", "laptop", "gamingLaptop", "pcLaptop"];
    let response;

    for (const contentType of possibleTypes) {
      try {
        const query: ContentfulQuery = {
          content_type: contentType,
          include: 1,
        ...
        if (response.items.length > 0) {
          logger.debug(
            `📦 Found ${response.items.length} laptops from CMS using content type: ${contentType}`
          );
          break;
        } else {
          logger.debug(`No items found for content type: ${contentType}`);
        }
      } catch (typeError) {
        logger.debug(
          `Content type '${contentType}' not found or error: ${
    ...
    if (!response || response.items.length === 0) {
      logger.warn(
        `⚠️ No laptop content found in Contentful. Tried content types: ${possibleTypes.join(
          ", "
        )}`
      );
      setCache(cacheKey, []);
    ...
    const items: import("../components/PCBuilder/types").PCBuilderComponent[] =
      response.items.map((item) => {
        ...
        const rawEan =
          getNumber(fields.ean) ??
          getString(fields.ean) ??
          getString((fields as Record<string, unknown>)["EAN"]) ??
          getNumber((fields as Record<string, unknown>)["EAN"]);
        const ean = rawEan !== undefined ? String(rawEan) : undefined;

        // Tech sheet link (direct URL or asset reference)
        let techSheet: string | undefined;
        const techSheetField =
          (fields as Record<string, unknown>)["techSheet"] ??
          (fields as Record<string, unknown>)["techsheet"] ??
          (fields as Record<string, unknown>)["tech_sheet"] ??
          (fields as Record<string, unknown>)["datasheet"];

        if (typeof techSheetField === "string") {
          techSheet = techSheetField.startsWith("//")
            ? `https:${techSheetField}`
            : techSheetField;
        } else if (
          techSheetField &&
          typeof techSheetField === "object" &&
          "fields" in techSheetField &&
          (techSheetField as { fields?: { file?: { url?: string } } })?.fields
            ?.file?.url
        ) {
          const url = (
            techSheetField as { fields: { file?: { url?: string } } }
          ).fields.file?.url;
          techSheet = url
            ? url.startsWith("//")
              ? `https:${url}`
        ...
        const pcComp: import("../components/PCBuilder/types").PCBuilderComponent =
          {
            ...
            battery:
              ...
              undefined,
            ports:
              getString((fields as Record<string, unknown>)["ports"]) ??
              undefined,
            coolingSolution:
              getString(
                (fields as Record<string, unknown>)["coolingSolution"]
              ) ?? undefined,
            wifi:
              getString((fields as Record<string, unknown>)["wifi"]) ??
              undefined,
            bluetooth:
              getString((fields as Record<string, unknown>)["bluetooth"]) ??
              undefined,
            operatingSystem:
              getString(
                (fields as Record<string, unknown>)["operatingSystem"]
              ) ?? undefined,
            techSheet,
            inStock: getBoolean(fields.inStock) ?? true,
            stockLevel: getNumber(fields.stockLevel) ?? undefined,
