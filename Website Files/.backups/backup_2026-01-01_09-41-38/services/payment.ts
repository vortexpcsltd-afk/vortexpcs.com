...
import { getCsrfToken, fetchCsrfTokenFromServer } from "../utils/csrfToken";
...
export const createPaymentIntent = async (
  ...
  try {
    ...
    try {
      ...
      // Get CSRF token - use stored token or fetch fresh one
      let csrfToken = getCsrfToken();
      if (!csrfToken) {
        logger.debug("No CSRF token in storage, fetching fresh token");
        csrfToken = await fetchCsrfTokenFromServer();
      }

      if (csrfToken) {
        headers["x-csrf-token"] = csrfToken;
        logger.info("CSRF token prepared for payment intent", {
          tokenLength: csrfToken.length,
          source: getCsrfToken() ? "storage" : "server",
        });
      } else {
        logger.error("Critical: Unable to get CSRF token for payment");
        throw new Error(
          "Security validation failed. Please refresh the page and try again."
        );
      }

      logger.debug("Making payment intent request", {
      ...
      const response = await retryOperation(
        () =>
          axios.post(
            ...
            {
              headers,
              withCredentials: true, // Include cookies in POST request
            }
          ),
