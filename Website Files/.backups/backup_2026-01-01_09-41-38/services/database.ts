...
// Fallback: fetch orders via admin API (server-side privileges) when client Firestore is denied
const fetchOrdersViaAdminApi = async (limitCount: number): Promise<Order[]> => {
  try {
    const user = auth?.currentUser;
    if (!user || typeof user.getIdToken !== "function") {
      logger.warn("fetchOrdersViaAdminApi: no authenticated user");
      return [];
    }

    const token = await user.getIdToken(true);
    const resp = await fetch(`/api/admin/orders/list?limit=${limitCount}`, {
      headers: {
    ...
    if (!resp.ok) {
      ...
      throw new Error(
        `Admin orders API failed (${resp.status}): ${text || "unknown error"}`
      );
    }
    ...
    return list.map((raw) => ({
      ...
      orderDate: raw.orderDate ? new Date(String(raw.orderDate)) : undefined,
      estimatedCompletion: raw.estimatedCompletion
        ? new Date(String(raw.estimatedCompletion))
        : undefined,
      deliveryDate: raw.deliveryDate
        ? new Date(String(raw.deliveryDate))
        : undefined,
    })) as Order[];
  } catch (err) {
    logger.error("fetchOrdersViaAdminApi failed", err);
    return [];
  }
};
...
export interface Order {
  ...
  items: Array<{
    ...
    quantity: number;
    price: number;
    ean?: string; // European Article Number for picking/packing
  }>;
  total: number;
  status:
    | "pending"
...
/**
 * Vortex Vault - Loyalty & Rewards System
 */
export interface VortexVaultAccount {
  id?: string;
  userId: string;
  currentBalance: number; // Current points available
  lifetimePoints: number; // Total points ever earned
  pointsRedeemed: number; // Total points redeemed
  pointsExpired: number; // Total points expired
  lastActivityDate: Date; // For expiry tracking
  autoApplyPreference?: boolean; // Whether to auto-apply points at checkout
  createdAt: Date;
  updatedAt: Date;
}

export interface VortexVaultTransaction {
  id?: string;
  userId: string;
  orderId?: string;
  type: "purchase" | "bonus" | "review" | "referral" | "redemption" | "expiry";
  pointsAmount: number; // Positive for earning, negative for spending
...
export interface VortexVaultConfig {
  id?: string;
  enabled: boolean;
  pointsPerPound: number; // Default: 0.1 pts/£ (1 pt per £10)
  pointsValueConversion: number; // Default: 10 pts = £1
  minimumRedemptionPoints: number; // Default: 50 pts (£5)
  bonusPointsSignup: number; // Default: 50
  bonusPointsReview: number; // Default: 25
  bonusPointsReferral: number; // Default: 100
  pointsExpiryMonths: number; // Default: 18
  createdAt: Date;
  updatedAt: Date;
}
...
export const getAllOrders = async (
  ...
  try {
    ...
    // If Firestore returned no orders but we are authenticated, try admin API fallback
    if (orders.length === 0) {
      const fallback = await fetchOrdersViaAdminApi(limitCount);
      if (fallback.length) {
        logger.warn(
          "getAllOrders: Firestore returned empty; using admin API fallback",
    ...
    if (isPermissionError) {
      logger.warn("getAllOrders permission denied; attempting admin API");
      const fallback = await fetchOrdersViaAdminApi(limitCount);
...
export const getAllOrdersExtended = async (
  ...
  try {
    ...
    if (merged.length === 0) {
      const fallback = await fetchOrdersViaAdminApi(limitPrimary);
      if (fallback.length) {
        logger.warn(
          "getAllOrdersExtended: Firestore returned empty; using admin API fallback",
          { count: fallback.length }
        );
        return fallback;
      }
    ...
    if (isPermissionError) {
      logger.warn(
        "getAllOrdersExtended permission denied; attempting admin API"
      );
      const fallback = await fetchOrdersViaAdminApi(limitPrimary);
      if (fallback.length) return fallback;
      return [];
