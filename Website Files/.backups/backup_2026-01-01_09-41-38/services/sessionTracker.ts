...
/**
 * Initialize session tracking with proper cleanup tracking
 */
export function initializeSessionTracking(userId?: string) {
  // Prevent duplicate initialization
  if (trackingActive) {
    logger.info("📊 [SessionTracker] Session tracking already active", {
      sessionId,
    });
    return sessionId;
  }
...
/**
 * Setup activity tracking with cleanup tracking
 */
function setupActivityTracking() {
  ...
  // Track various user interactions (throttled) - use tracked listeners
  addTrackedListener(document, "click", updateActivity);
  addTrackedListener(document, "scroll", updateActivity);
  addTrackedListener(document, "keypress", updateActivity);
  addTrackedListener(document, "mousemove", updateActivity);
  ...
  addTrackedListener(document, "click", resetInactivityTimer);
  addTrackedListener(document, "scroll", resetInactivityTimer);
  addTrackedListener(document, "keypress", resetInactivityTimer);
  addTrackedListener(document, "mousemove", resetInactivityTimer);
...
function setupFrustrationTracking() {
  ...
  const frustrationClickHandler = (e: Event) => {
    const now = Date.now();
  ...
  addTrackedListener(document, "click", frustrationClickHandler, {
    capture: true,
  });
}
...
/**
 * Stop session tracking and cleanup all resources
 * CRITICAL: Call this on component unmount to prevent memory leaks
 */
export function stopSessionTracking() {
  ...
  logger.debug("📊 [SessionTracker] Session tracking stopped and cleaned up");
}

/**
 * Start session tracking with automatic cleanup
 * Returns cleanup function for use in useEffect
 */
export function startSessionTracking(userId?: string) {
  initializeSessionTracking(userId);
