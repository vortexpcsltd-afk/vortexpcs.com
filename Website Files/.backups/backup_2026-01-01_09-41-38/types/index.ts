 ...
 * Consolidated from multiple sources to be the single source of truth
 * Includes all fields that might be used across the application
 */
export interface CartItem {
  ...
  reducedPrice?: number; // Sale/discounted price (alternative to using originalPrice)
 ...
 * Consolidated to support both internal (line1/county) and API (street/state) naming
 * Both naming conventions are supported for backward compatibility
 */
export interface ShippingAddress {
  // Street address - supports both 'street' (API) and 'line1' (form) names
  street?: string;
  line1?: string;
  ...
  // Postcode - UK format validation required by API
  postcode: string;

  // Country - 2-character code (e.g., "GB", "US")
  country: string;

  // State/county - supports both 'state' (API) and 'county' (form) names
  state?: string;
  county?: string;
}
