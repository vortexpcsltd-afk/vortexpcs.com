...
import { VortexVaultBalanceChip } from "./components/VortexVaultBalanceChip";
import { VortexVaultFAQModal } from "./components/VortexVaultFAQModal";
...
export default function App() {
  ...
  // Pending referral count badge state
  ...
  useEffect(() => {
    ...
    import("./services/vortexVaultReferrals").then(
      ({ getReferralCodeFromURL, storeReferralCode }) => {
        const refCode = getReferralCodeFromURL();
        if (refCode) {
          storeReferralCode(refCode);
          logger.info("Referral code detected and stored", { code: refCode });

          // Track referral link click for analytics
          import("./services/database").then(({ trackEvent }) => {
            trackEvent("referral_link_clicked", {
              ...
              logger.warn("Failed to track referral click", err);
            });
          });
        }
      }
    ...
    const cleanupSession = startSessionTracking(user?.uid);
  ...
  // Load pending referral count when user is authenticated
  useEffect(() => {
    ...
    const loadReferralCount = async () => {
      try {
        const { getReferralStats } = await import(
          "./services/vortexVaultReferrals"
        );
        const stats = await getReferralStats(user.uid);
        if (stats) {
          // Count only pending referrals
          const pendingCount =
            stats.referrals?.filter((ref) => ref.status === "pending").length ||
            0;
          setPendingReferralCount(pendingCount);
        }
      } catch (error) {
        logger.debug("Failed to load referral count", error);
        // Silently fail - don't show errors for badge
      }
    };
  ...
  useEffect(() => {
    ...
    try {
      ...
      const sid = initializeSessionTracking(uid || undefined);
      logger.info("📊 [App] Session initialized", { sessionId: sid });

      // Track page view
      const page = `/${currentView}`;
      logger.info("📊 [App] About to track page", { page, uid });
      // Pass undefined instead of null to satisfy type signature
      trackPage(page, document.title, uid || undefined);
  ...
  // Cross-tab synchronization disabled for now; relying on in-app state.
  // In dev, same-tab storage events can fire unexpectedly and reload the cart
  // mid-write, leading to optional fields (like EAN) being dropped.
  // useEffect(() => {
  //   const handleStorageChange = async (event: StorageEvent) => {
  //     console.log(`[App.tsx storage event] key=${event.key}, triggered reload`);
  //     if (event.key === "vortex_cart" && event.newValue !== event.oldValue) {
  //       try {
  //         const updatedCart = await loadCart();
  //         console.log(
  //           `[App.tsx storage event] Loaded cart from storage:`,
  //           updatedCart.map((i) => ({ id: i.id, hasEan: 'ean' in i, ean: i.ean }))
  //         );
  ...
  useEffect(() => {
    ...
    logger.debug(`[App.tsx useEffect] cartItems changed, about to persist:`, {
  ...
  const addToCart = (item: CartItem) => {
    ...
    logger.debug(`[App.tsx] addToCart called with:`, {
      id: item.id,
      name: item.name,
      ...
      try {
        ...
        if (existingItem) {
          ...
          const newItem: CartItem = {
            ...
            originalPrice: item.originalPrice, // Preserve original price for savings calculation
            reducedPrice: item.reducedPrice, // Preserve reduced price if provided
          };

          logger.debug(`[App.tsx] Created newItem for cart:`, { newItem });
          const newCartArray = [...prevItems, newItem];
          logger.debug(`[App.tsx] Returning new cart array:`, {
            items: newCartArray.map((i) => ({
              id: i.id,
              name: i.name,
              hasEan: "ean" in i,
              ean: i.ean,
  ...
  const navigation = [
    ...
    { id: "vortex-vault", label: "Rewards", icon: Gift },
                ...
                <div className="relative z-10">
                  ...
                  >
                    <div className="container mx-auto px-4 md:px-6 lg:px-8">
                      {/* Top Row: Auth & Cart (Desktop) */}
                      <div
                        className="hidden md:flex items-center justify-between relative"
                        style={{ paddingTop: "16px", paddingBottom: "16px" }}
                      >
                        {/* Gradient fade border */}
                        <div className="absolute bottom-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent from-75% to-cyan-400/60"></div>
                        <div></div>
                        {/* Right Actions Row */}
                        <div className="flex items-center space-x-2">
                          ...
                          {isLoggedIn ? (
                            <>
                              <Button
                                ...
                                className="text-green-400 hover:text-green-300 px-3 py-1 text-xs focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-green-400 focus-visible:ring-offset-2 focus-visible:ring-offset-black relative"
                              >
                                <User className="w-3.5 h-3.5 mr-1.5" />
                                Account
                                {pendingReferralCount > 0 && (
                                  <span className="absolute top-0 right-0 inline-flex items-center justify-center px-1.5 py-0.5 text-xs font-bold leading-none text-white transform translate-x-1/2 -translate-y-1/2 bg-cyan-500 rounded-full">
                                    {pendingReferralCount > 9
                                      ? "9+"
                              ...
                              {isAdmin && (
                                <>
                                  <Button
                                    ...
                                    className="text-red-400 hover:text-red-300 px-3 py-1 text-xs focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-400 focus-visible:ring-offset-2 focus-visible:ring-offset-black"
                                  >
                                    Admin
                                  </Button>
                                  <Button
                                    ...
                                    className="text-sky-400 hover:text-sky-300 px-3 py-1 text-xs focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-400 focus-visible:ring-offset-2 focus-visible:ring-offset-black"
                                  >
                                    CMS
                                  </Button>
                                </>
                            ...
                            <>
                              <Button
                                ...
                                className="relative text-sky-400 hover:text-sky-300 hover:bg-sky-500/10 px-3 py-1 text-xs group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-400 focus-visible:ring-offset-2 focus-visible:ring-offset-black before:pointer-events-none before:absolute before:left-0 before:right-0 before:-bottom-[8px] before:h-[3px] before:rounded-full before:bg-gradient-to-r before:from-transparent before:via-sky-400 before:to-transparent before:opacity-100 before:shadow-[0_0_8px_rgba(56,189,248,0.6)] before:transition-all before:duration-300 hover:before:via-cyan-300 hover:before:shadow-[0_0_12px_rgba(103,232,249,0.8)]"
                              >
                                <LogIn className="w-3.5 h-3.5 mr-1 relative z-10" />
                              ...
                              <Button
                                ...
                                className="relative bg-gradient-to-r from-sky-600 to-blue-600 hover:from-sky-500 hover:to-blue-500 text-white px-3 py-1 text-xs shadow-lg shadow-sky-500/30 hover:shadow-sky-500/50 transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-400 focus-visible:ring-offset-2 focus-visible:ring-offset-black before:pointer-events-none before:absolute before:left-0 before:right-0 before:-bottom-[8px] before:h-[3px] before:rounded-full before:bg-gradient-to-r before:from-transparent before:via-cyan-300 before:to-transparent before:opacity-100 before:shadow-[0_0_8px_rgba(103,232,249,0.6)] before:transition-all before:duration-300 hover:before:via-white hover:before:shadow-[0_0_12px_rgba(255,255,255,0.8)]"
                              >
                          ...
                          {/* Vault Balance Chip */}
                          {isLoggedIn && (
                            <VortexVaultBalanceChip
                              userId={user?.uid}
                              compact={true}
                              onClick={() => setShowVaultFAQ(true)}
                            />
                          )}
                          ...
                          <Button
                            ...
                            className="relative text-sky-400 hover:text-sky-300 hover:bg-sky-500/10 min-w-[40px] min-h-[40px] px-2 py-1 group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-400 focus-visible:ring-offset-2 focus-visible:ring-offset-black before:pointer-events-none before:absolute before:left-0 before:right-0 before:-bottom-[8px] before:h-[3px] before:rounded-full before:bg-gradient-to-r before:from-transparent before:via-sky-400 before:to-transparent before:opacity-100 before:shadow-[0_0_8px_rgba(56,189,248,0.6)] before:transition-all before:duration-300 hover:before:via-cyan-300 hover:before:shadow-[0_0_12px_rgba(103,232,249,0.8)]"
                          >
                            <ShoppingCart className="w-4 h-4 relative z-10" />
                            {cartItems.length > 0 && (
                              <span className="absolute -top-1 -right-1 w-4 h-4 bg-gradient-to-r from-sky-500 to-blue-500 text-white text-xs rounded-full flex items-center justify-center shadow-lg shadow-sky-500/50 z-20 font-bold">
                      ...
                      {/* Main Navigation Row */}
                      <div
                        className="flex items-center justify-between h-20 md:h-20"
                      ...
                      >
                        ...
                        {/* Right Actions - Mobile Cart and Menu */}
                        <div className="flex items-center space-x-3">
                          {/* Shopping Cart - Visible on all sizes */}
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => setShowCartModal(true)}
                            aria-label="Shopping cart"
                            className="md:hidden relative text-sky-400 hover:text-sky-300 hover:bg-sky-500/10 min-w-[44px] min-h-[44px] px-3 py-3 group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-400 focus-visible:ring-offset-2 focus-visible:ring-offset-black"
                          >
                        ...
                        <div className="container mx-auto px-4 md:px-6 lg:px-8 py-5">
                          ...
                          <div className="flex flex-col space-y-2.5 mb-5">
                            ...
                            <button
                              ...
                              aria-current={
                                currentView === "home" ? "page" : undefined
                              }
                          ...
                          {isLoggedIn ? (
                            <div className="flex flex-col space-y-2.5">
                              <button
                                ...
                                className="flex items-center space-x-2.5 px-5 py-4 text-green-400 hover:text-green-300 hover:bg-green-500/10 rounded-lg transition-all min-h-[44px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-green-400 focus-visible:ring-offset-2 focus-visible:ring-offset-black relative"
                              >
                                <User className="w-5 h-5" />
                                <span>My Account</span>
                                {pendingReferralCount > 0 && (
                                  <span className="absolute right-5 inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-white bg-cyan-500 rounded-full">
                                    {pendingReferralCount > 9
                                      ? "9+"
                                      : pendingReferralCount}
                                  </span>
