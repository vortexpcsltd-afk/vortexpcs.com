...
import { ensureCsrfToken } from "../utils/csrfToken";
...
export function CheckoutPage({
  ...
  // Fetch CSRF token on mount - CRITICAL for payment processing
  // This ensures token is available before user attempts payment
  useEffect(() => {
    let isMounted = true;
    ...
    (async () => {
      try {
        ...
        if (isMounted && !token) {
          logger.warn("CSRF token fetch failed on mount - payment may fail");
          // Don't show error to user yet, try again at payment time
        } else if (isMounted && token) {
          logger.debug("CSRF token acquired on mount");
        }
      } catch (error) {
        if (isMounted) {
          logger.error("Error ensuring CSRF token on mount:", error);
        }
      }
    })();
  ...
  const [vaultRedeemablePoints, setVaultRedeemablePoints] = useState(0);
  const [vaultDiscountValue, setVaultDiscountValue] = useState(0);
  const [vaultAutoApply, setVaultAutoApply] = useState(true);
  ...
  const totalSaved = cartItems.reduce((sum, item) => {
    ...
    const savings = Math.max(0, itemOriginalPrice - item.price) * item.quantity;
    return sum + savings;
  }, 0);
  ...
  const loyaltyDiscount = vaultAutoApply
    ? Math.min(vaultDiscountValue || 0, Math.max(0, finalSubtotal))
    : 0;
  const total = Math.max(0, finalSubtotal - loyaltyDiscount) + shippingCost;
  ...
  // Load Vortex Vault balance for redemption
  useEffect(() => {
    const loadVault = async () => {
      try {
        ...
        setVaultRedeemablePoints(0);
        setVaultDiscountValue(0);
        ...
        const summary = await getVortexVaultSummary(uid);
        const balance = summary.account?.currentBalance ?? 0;
        const redeemable =
          balance >= MINIMUM_REDEMPTION_POINTS
            ? Math.floor(balance / MINIMUM_REDEMPTION_POINTS) *
              MINIMUM_REDEMPTION_POINTS
            : 0;

        setVaultBalance(balance);
        setVaultRedeemablePoints(redeemable);
        setVaultDiscountValue(calculatePointsValue(redeemable));
        setVaultAutoApply(summary.autoApplyPreference ?? true);
      } catch (err) {
        logger.error("Failed to load Vortex Vault balance", err);
        setVaultError("Unable to load rewards right now");
      } finally {
  ...
  const handleVaultApplyToggle = async () => {
    const nextValue = !vaultAutoApply;
    setVaultAutoApply(nextValue);
    ...
    try {
      ...
      const result = await setVortexVaultAutoApply(uid, nextValue);
      if (!result.success) {
        setVaultAutoApply(!nextValue);
        toast.error("Couldn't save your preference. Try again.");
      }
    } catch (err) {
      logger.error("Failed to persist vault auto-apply preference", err);
      setVaultAutoApply(!nextValue);
      toast.error("Couldn't save your preference. Try again.");
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const orderData = {
        amount: total,
        currency: "gbp",
        loyaltyPointsApplied: vaultAutoApply ? vaultRedeemablePoints : 0,
        loyaltyDiscount: loyaltyDiscount,
        loyaltyBalance: vaultBalance ?? undefined,
  ...
  const processStripePayment = async (
    ...
    try {
      ...
      // CRITICAL: Ensure CSRF token is available before payment attempt
      // This prevents the "Missing CSRF token" error that kills conversions
      let csrfToken = await ensureCsrfToken();
      if (!csrfToken) {
        logger.error(
          "CRITICAL: Unable to obtain CSRF token for payment - aborting"
        );
        throw new Error(
      ...
      if (!response.ok) {
        ...
        try {
          ...
          if (response.status === 403) {
            ...
            // CSRF failure - attempt ONE retry with fresh token
            if (retryCount < 1) {
              logger.warn(
                "CSRF validation failed, attempting retry with fresh token",
                {
              ...
              if (freshToken && freshToken !== csrfToken) {
                logger.info("Retrying payment with fresh CSRF token");
                // Recursive call with fresh context
                return processStripePayment(orderData, authToken);
              }
            }
  ...
  const processPayPalPayment = async (
    ...
    // CRITICAL: Ensure CSRF token before payment
    ...
    const userIdForPayPal =
      typeof window !== "undefined"
        ? sessionStorage.getItem("vortex_user_id") || undefined
        : undefined;
    const paypalData = {
      items: orderData.cartItems,
      customerEmail: orderData.customerEmail,
      currency: orderData.currency,
      loyaltyPointsApplied: orderData.loyaltyPointsApplied,
      loyaltyDiscount: orderData.loyaltyDiscount,
      loyaltyBalance: orderData.loyaltyBalance,
      metadata: {
        shippingAddress: orderData.shippingAddress,
        customerName: orderData.customerName,
        customerPhone: orderData.customerPhone,
        userId: userIdForPayPal,
                  ...
                  {!stripeClientSecret && (
                    ...
                    >
                      {!isOnline ? (
                        ...
                        <>
                          <Lock className="w-5 h-5 mr-2" />
                          Complete Secure Payment - {formatCurrency(total)}
                        </>
                      )}
                    </ButtonWithLoading>
                  )}
              ...
              <div className="space-y-2 mb-4">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-white">
                    {formatCurrency(componentsSubtotal)}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-400">Custom Build Service</span>
                  <span className="text-white">
                    {formatCurrency(buildServiceCost)}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
                ...
                {/* Total Saved - Show if there are discounts */}
                {totalSaved > 0 && (
                  <div className="flex justify-between text-sm bg-green-500/10 border border-green-500/30 rounded p-2.5">
                ...
                {/* Vortex Vault Redemption */}
                <div className="space-y-2 p-3 rounded-md bg-white/5 border border-white/10">
                  <p className="text-xs uppercase tracking-wide text-gray-400 flex items-center justify-between">
                    <span>Vortex Vault</span>
                  ...
                  {vaultLoading ? (
                    ...
                    <p className="text-xs text-amber-400">{vaultError}</p>
                  ) : vaultBalance !== null ? (
                    <div className="space-y-2">
                      <div className="flex items-center justify-between gap-3">
                        <div className="text-sm text-gray-200">
                          ...
                          = £{calculatePointsValue(vaultBalance).toFixed(2)}
                        </div>
                        <Button
                          variant={vaultAutoApply ? "default" : "outline"}
                          size="sm"
                          onClick={handleVaultApplyToggle}
                          className={
                            vaultAutoApply
                              ? "h-8 bg-gradient-to-r from-cyan-600 to-blue-600"
                              : "h-8"
                          }
                        >
                          {vaultAutoApply ? "Applied" : "Apply"}
                        </Button>
                      </div>
                      {vaultRedeemablePoints >= MINIMUM_REDEMPTION_POINTS ? (
                        <div className="text-xs text-gray-300">
                          {vaultAutoApply ? (
                            <>
                              Applying £{vaultDiscountValue.toFixed(2)} using{" "}
                              {vaultRedeemablePoints} pts.
                              <div className="text-[11px] text-cyan-300 mt-1">
                                You will keep £
                                {calculatePointsValue(
                                  Math.max(
                                    vaultBalance - vaultRedeemablePoints,
                                    0
                                  )
                                ).toFixed(2)}{" "}
                                available for later.
                            ...
                            <>
                              You can apply £{vaultDiscountValue.toFixed(2)}{" "}
                              now.
                              <div className="text-[11px] text-cyan-300 mt-1">
                                Toggle apply to redeem {vaultRedeemablePoints}{" "}
                                pts on this order.
                              </div>
                            </>
                          )}
                        ...
                        <div className="text-xs text-gray-400">
                          Earn{" "}
                          {Math.max(
                            MINIMUM_REDEMPTION_POINTS - vaultBalance,
                            0
                          )}{" "}
                          more pts to unlock a £
                          {calculatePointsValue(
                            MINIMUM_REDEMPTION_POINTS
                          ).toFixed(2)}{" "}
                          redemption.
