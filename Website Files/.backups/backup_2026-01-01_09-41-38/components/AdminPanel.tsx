...
import { AdminVortexVaultSection } from "./admin/AdminVortexVaultSection";
...
const normalizeOrders = (orders: Order[]): ExtendedOrder[] => {
  return orders.map((orig) => {
    ...
    const normalizedItems: Order["items"] = rawItems.map((i) => ({
      ...
      ean: (i as Record<string, unknown>).ean as string | undefined,
    }));
    const computedTotal =
            ...
            >
                            ...
                            if (shipping > 0) {
                              ...
                              return (
                                <p className="text-xs text-gray-500 mt-1">
                                  Items: {formatCurrency(itemsOnly)} + Shipping:{" "}
                                ...
                                >
                                  ...
                                  <td className="p-2 text-gray-300">
                                    <div className="space-y-1">
                                      {(it as Record<string, unknown>).ean && (
                                        <p className="font-mono text-xs">
                                          EAN:{" "}
                                          {(it as Record<string, unknown>).ean}
                                        </p>
                                      )}
                                      {(it as Record<string, unknown>).sku && (
                                        <p className="font-mono text-xs">
                                          SKU:{" "}
                                          {(it as Record<string, unknown>).sku}
                                        </p>
              ...
              <TabsContent value="vault" className="space-y-6">
                {!isAdmin && (
                  <Card className="bg-gradient-to-r from-amber-500/10 to-red-500/10 border-amber-500/30 backdrop-blur-xl p-4">
                    <div className="text-sm text-amber-300">
                      Admin access required to manage Vortex Vault.
                    </div>
                  </Card>
                )}
                {isAdmin && (
                  <ComponentErrorBoundary>
                    <AdminVortexVaultSection />
                  </ComponentErrorBoundary>
                )}
              </TabsContent>
            </Tabs>
