...
export const ProductionSheet: React.FC<ProductionSheetProps> = ({
    ...
    order.items.forEach((i) => {
      ...
      lines.push(
        [
          esc(i.name),
          esc(i.category || ""),
          esc((i as Record<string, unknown>).ean || ""),
          esc((i as Record<string, unknown>).sku || ""),
          esc(i.quantity),
  ...
  const exportPDF = async () => {
    try {
      ...
      doc.text("EAN/SKU", left + 75, y);
      doc.text("Cat", left + 110, y);
      ...
      order.items.forEach((i, idx) => {
        ...
        doc.setFontSize(8);
        const componentText =
          i.name.length > 35 ? i.name.slice(0, 32) + "..." : i.name;
        doc.text(componentText, left + 2, y);

        // EAN/SKU display
        const ean = (i as Record<string, unknown>).ean as string | undefined;
        const sku = (i as Record<string, unknown>).sku as string | undefined;
        const eanSkuText = ean || sku || "—";
        doc.setFontSize(7);
        doc.text(
          eanSkuText.length > 15 ? eanSkuText.slice(0, 12) + "..." : eanSkuText,
          left + 75,
          y
        );
        doc.setFontSize(8);

        doc.text((i.category || "").slice(0, 10), left + 110, y);
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-gray-100">
                ...
                <th className="border border-gray-400 px-2 py-2 text-left text-sm w-32">
                  EAN / SKU
                </th>
                ...
                <tr key={index}>
                  ...
                  <td className="border border-gray-400 px-2 py-2 text-sm">
                    <div className="flex flex-col gap-0.5">
                      {(item as Record<string, unknown>).ean && (
                      ...
                      {(item as Record<string, unknown>).sku && (
                        <span className="text-[10px] text-gray-600">
                          SKU: {(item as Record<string, unknown>).sku}
                        </span>
                      )}
                      {!(item as Record<string, unknown>).ean &&
                        !(item as Record<string, unknown>).sku && (
