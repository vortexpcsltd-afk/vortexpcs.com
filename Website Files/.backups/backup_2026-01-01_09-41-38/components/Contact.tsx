...
export function Contact({ onNavigate }: ContactProps = {}) {
  ...
  const nameError = touched.name ? getNameError(formData.name, true) : null;
  const emailError = touched.email ? getEmailError(formData.email, true) : null;
  const phoneError = touched.phone
    ? getPhoneError(formData.phone, false)
    : null;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch("/api/contact/send", {
        ...
        logger.warn("Contact form request failed", { error });
        return null;
      });

      if (!response) {
        throw new Error(
          "Unable to reach our contact service. Please email info@vortexpcs.com."
        );
      }
