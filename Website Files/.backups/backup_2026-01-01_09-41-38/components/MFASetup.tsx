...
export function MFASetup({ onComplete }: MFASetupProps) {
  ...
  const recaptchaReadyRef = React.useRef<Promise<void> | null>(null);
  const recaptchaVerifierRef = React.useRef<RecaptchaVerifier | null>(null);

  // Ensure reCAPTCHA script is loaded and ready before executing
  const ensureRecaptchaReady = React.useCallback(() => {
    if (!recaptchaSiteKey) {
  ...
  const executeRecaptcha = async (): Promise<string> => {
    if (!recaptchaSiteKey) {
      logger.error("reCAPTCHA site key missing");
  ...
  const sendSMSCode = async () => {
    ...
    try {
      ...
      if (!recaptchaVerifierRef.current) {
        ...
        // Ensure the widget renders at least once; Firebase may reuse it for invisible mode
