              ...
              <div className="text-right bg-gradient-to-br from-sky-500/20 to-blue-500/20 px-8 py-6 rounded-xl border-2 border-sky-400/40">
                ...
                <div className="flex items-start justify-end gap-1">
                  ...
                  <span className="text-2xl font-bold text-sky-300 mt-1">
                    .{(component.price ?? 0).toFixed(2).split(".")[1]}
                  </span>
                </div>
                {component.brand && (
      ...
      >
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="space-y-3">
            ...
            <p className="text-gray-400 text-sm">{peripheral.description}</p>
...
export function PCBuilder({
  ...
  const activeComponentData: ComponentDataMap = useCmsData
    ? cmsComponents
    : componentData;
  ...
  const getTotalPrice = () => {
    const componentsTotal = Object.entries(selectedComponents).reduce(
      (total, [category, componentId]) => {
        ...
        return total + (component ? component.price ?? 0 : 0);
      },
      0
    );
  ...
  const generateBuildComments = () => {
    ...
    // Cooling insights
    if (cooling && cpu) {
      // Guard cooling.name access with fallback string
      const coolingName = typeof cooling.name === "string" ? cooling.name : "";
      if (cooling.type === "Liquid" && coolingName.includes("360mm")) {
        comments.push(
      ...
      } else if (cooling.type === "Liquid" && coolingName.includes("280mm")) {
        comments.push(
