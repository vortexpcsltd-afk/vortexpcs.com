...
// PDF & Excel generators are available in api/package.json deps
...
async function verifyAdmin(
  ...
  try {
    ...
    // Minimal allowlist-based admin gate to match schedule.ts behaviour
    ...
    const isAdmin = claimsRole === "admin" || allow.has(email);
...
function parseParams(req: VercelRequest) {
  const q = req.query || {};
  const format = (Array.isArray(q.format) ? q.format[0] : q.format) as
    | ReportFormat
    | undefined;
  const startDate = Array.isArray(q.startDate)
    ? q.startDate[0]
    : (q.startDate as string | undefined);
  const endDate = Array.isArray(q.endDate)
    ? q.endDate[0]
    : (q.endDate as string | undefined);
  const metricsStr = Array.isArray(q.metrics)
    ? q.metrics[0]
    : (q.metrics as string | undefined);
  const metrics = (metricsStr || "").split(",").filter(Boolean);
  return { format: format || "pdf", startDate, endDate, metrics };
}

function generateSampleData(metrics: string[]) {
  const cols = ["Metric", "Value", "Notes"];
  const rows = (
    metrics.length ? metrics : ["sessions", "orders", "revenue"]
  ).map((m) => [
    m,
...
function buildPdfBuffer(params: {
  ...
  doc.setFontSize(16);
  doc.text(params.title, 40, 50);
  doc.setFontSize(10);
  doc.text(params.dateRange, 40, 70);
  autoTable(doc, {
    head: [params.cols],
    body: params.rows,
    startY: 90,
...
function buildExcelBuffer(params: {
  title: string;
  dateRange: string;
  cols: string[];
  rows: any[][];
  ...
  const header = [[params.title], [params.dateRange], []];
  const table = [params.cols, ...params.rows];
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const { format, startDate, endDate, metrics } = parseParams(req);
    const title = "Vortex PCs Analytics Report";
    const rangeText = `Range: ${startDate || "(unspecified)"} 	to ${
      endDate || "(unspecified)"
    }`;
    const { cols, rows } = generateSampleData(metrics);

    if (format === "excel") {
      const buf = buildExcelBuffer({ title, dateRange: rangeText, cols, rows });
      const filename = `vortex-report-${formatDateSafe(new Date())}.xlsx`;
      res.setHeader(
        "Content-Type",
    ...
    // default to PDF
    const buf = buildPdfBuffer({ title, dateRange: rangeText, cols, rows });
    const filename = `vortex-report-${formatDateSafe(new Date())}.pdf`;
...
function formatDateSafe(d: Date) {
  try {
...
function generatePDF(data: ReportData, startDate: Date, endDate: Date): Buffer {
  ...
  // Add logo and title in professional header layout
  if (logoData) {
    try {
      // Logo with correct 5.4:1 aspect ratio (750x139px original dimensions)
      const logoWidth = 54;
      const logoHeight = 10;
      const logoX = 15;
      const logoY = 15;
  ...
  // Analytics table using autoTable
  autoTable(doc, {
    startY: y,
    head: [["Metric", "Value"]],
    body: [
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    } else if (formatType === "excel" || formatType === "xlsx") {
