  ...
  async (req: VercelRequest, res: VercelResponse) => {
    try {
      ...
      if (!resCapture.ok) {
        ...
        return res.status(resCapture.status).json({
          ok: false,
        ...
        if (smtpHost && smtpUser && smtpPass) {
          ...
          if (payer?.email_address) {
            sendOps.push(
              sendEmailWithRetry(transporter, {
                ...
                .then((r) => {
                  if (!r.success)
                    throw r.error || new Error("Customer email failed");
                })
          ...
          sendOps.push(
            sendEmailWithRetry(transporter, {
              ...
              .then((r) => {
                if (!r.success)
                  throw r.error || new Error("Business email failed");
              })
          ...
          console.error(
            "SMTP configuration missing - emails skipped (PayPal)",
            {
              hasHost: !!smtpHost,
              hasUser: !!smtpUser,
              hasPass: !!smtpPass,
