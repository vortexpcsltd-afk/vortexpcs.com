...
export function AnalyticsDashboard() {
  ...
  const [ytdData, setYtdData] = useState<any>(null); // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  const fetchYTDSummary = async () => {
    ...
    try {
      const token = await auth.currentUser.getIdToken();
      const res = await fetch(`/api/admin/analytics/summary-ytd`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      if (!visitorsRes.ok || !visitors.success) {
        ...
        console.log("Visitor stats received:", visitors.data);
        console.log("Time series length:", visitors.data?.timeSeries?.length);
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
              <div className="bg-white/5 rounded-lg p-4">
                ...
                <div className="text-2xl font-bold text-white">
                  {monthlyData.summary.totalVisitors.toLocaleString()}
                </div>
                <div className="text-xs text-gray-500">
                  Avg:{" "}
                  {monthlyData.summary.avgVisitorsPerMonth.toLocaleString()}/mo
                </div>
              </div>
              <div className="bg-white/5 rounded-lg p-4">
                <div className="text-sm text-gray-400">Total Page Views</div>
                <div className="text-2xl font-bold text-white">
                  {monthlyData.summary.totalPageViews.toLocaleString()}
                </div>
              </div>
              <div className="bg-white/5 rounded-lg p-4">
                <div className="text-sm text-gray-400">Total Sessions</div>
                <div className="text-2xl font-bold text-white">
                  {monthlyData.summary.totalSessions.toLocaleString()}
                </div>
              </div>
              <div className="bg-white/5 rounded-lg p-4">
                <div className="text-sm text-gray-400">Total Revenue</div>
                <div className="text-2xl font-bold text-green-400">
                  £{monthlyData.summary.totalRevenue.toLocaleString()}
                </div>
                <div className="text-xs text-gray-500">
                  Avg: £
                  {monthlyData.summary.avgRevenuePerMonth.toLocaleString()}/mo
                </div>
              </div>
              <div className="bg-white/5 rounded-lg p-4">
                <div className="text-sm text-gray-400">Total Orders</div>
                <div className="text-2xl font-bold text-white">
                  {monthlyData.summary.totalOrders}
                </div>
              </div>
            </div>
                ...
                <div className="space-y-4">
                  ...
                  >
                    <div className="absolute inset-0 flex items-end justify-around px-4 pb-4 gap-3">
                      {(() => {
                        // Generate complete date range and fill in missing data
                        const now = new Date();
                        ...
                        if (visitorTrendPeriod === "7d") {
                          ...
                          startDate.setDate(now.getDate() - 6); // Today + 6 days back = 7 days total
                          for (let i = 0; i < 7; i++) {
                          ...
                          // YTD - use all available data from API
                          dateRange = visitorStats.timeSeries.map(
                            (p) => new Date(p.date)
                          );
                        }

                        // For YTD, just use the existing data
                        let filteredData;
                        if (visitorTrendPeriod === "ytd") {
                          filteredData = visitorStats.timeSeries;
                        } else {
                          // Create complete dataset with zeros for missing days
                          filteredData = dateRange.map((date) => {
                        ...
                        // YTD uses all data from the API
                        ...
                        return filteredData.map((point, index) => {
                          const heightPx = Math.max(
                            30,
                            (point.visitors / maxVisitors) * 200
                          );
                          ...
                          return (
                            ...
                            >
                              {/* Bar */}
                              <div
                                className="w-full bg-gradient-to-t from-sky-600 via-sky-500 to-blue-400 rounded-t-lg shadow-lg hover:shadow-sky-500/50 transition-all duration-300 cursor-pointer relative overflow-hidden"
                              ...
                              >
                                ...
                                {/* Tooltip */}
                                <div className="absolute -top-12 left-1/2 -translate-x-1/2 bg-gray-900 text-white px-3 py-2 rounded-lg shadow-xl opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap z-20 pointer-events-none">
                                ...
                                {/* Value label on bar */}
                                <div className="absolute top-2 left-0 right-0 text-center text-white text-xs font-bold opacity-0 group-hover:opacity-100 transition-opacity">
                                  {point.visitors}
                                </div>
                              </div>
                  ...
                  <div className="grid grid-cols-3 gap-4 pt-2">
                    {(() => {
                      // Use only existing data for stats (no zeros for missing days)
                      const now = new Date();
                      now.setHours(0, 0, 0, 0);
