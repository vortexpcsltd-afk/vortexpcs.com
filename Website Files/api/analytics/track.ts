...
export default withSecureMethod(
  ...
  async (req: VercelRequest, res: VercelResponse) => {
    ...
    // Wrap everything in outer try-catch to prevent 500 errors
    try {
      try {
        ...
        const { kind, payload } =
          ((body || req.body) as IncomingAnalytics) ||
          ({} as IncomingAnalytics);
        console.log("[Analytics API] Processing:", { kind });

        if (!kind || !payload) {
          console.warn("[Analytics API] Missing kind or payload - noop");
          // Gracefully accept but do not persist to avoid frontend errors
          return res.status(202).json({ success: false, noop: true });
        }
        ...
        if (kind === "session") {
          const sessionId =
            typeof (payload as Record<string, unknown>).sessionId === "string"
              ...
              : undefined;
          if (!sessionId) {
            console.warn("[Analytics API] Missing sessionId - noop");
            return res.status(202).json({ success: false, noop: true });
          }
        ...
        console.error("Analytics track error (inner):", err);
        // Return 202 noop with diagnostics to reduce console noise
      ...
      // Outer catch for any errors before inner try-catch
      const err = outerError as ApiError;
      console.error("Analytics track error (outer):", err);
      return res.status(202).json({
        success: false,
        noop: true,
