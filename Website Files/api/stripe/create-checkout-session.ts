  ...
  try {
    ...
    const checkoutMetadata = {
      ...
      ...(sanitizedMetadata && typeof sanitizedMetadata === "object"
        ...
        : {}),
      cart: cartSerialized,
      // Loyalty metadata (if caller provided via sanitizedMetadata or body)
      loyaltyPointsApplied:
        (sanitizedMetadata as any)?.loyaltyPointsApplied ??
        (typeof req.body?.loyaltyPointsApplied === "number"
          ? String(req.body.loyaltyPointsApplied)
          : String(0)),
      loyaltyDiscount:
        (sanitizedMetadata as any)?.loyaltyDiscount ??
        (typeof req.body?.loyaltyDiscount === "number"
          ? String(req.body.loyaltyDiscount)
          : String(0)),
      loyaltyBalance:
        (sanitizedMetadata as any)?.loyaltyBalance ??
        (typeof req.body?.loyaltyBalance === "number"
          ? String(req.body.loyaltyBalance)
          : undefined),
