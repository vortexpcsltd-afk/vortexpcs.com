...
// BuildSummary extracted; integration pending
// import { BuildSummary } from "./PCBuilder/BuildSummary";
import { CompatibilityChecker } from "./PCBuilder/CompatibilityChecker";
import { EnvironmentSettings } from "./PCBuilder/EnvironmentSettings";
import { ComponentImageGallery } from "./PCBuilder/ComponentImageGallery";
...
// Skeleton Loading Components (memoized to prevent re-renders)
const ComponentCardSkeleton = memo(
  ({ viewMode = "grid" }: { viewMode?: string }) => {
    if (viewMode === "list") {
      return (
...
const BuildSummarySkeleton = memo(() => {
  return (
...
BuildSummarySkeleton.displayName = "BuildSummarySkeleton";

const CategoryNavSkeleton = memo(() => {
  return (
...
CategoryNavSkeleton.displayName = "CategoryNavSkeleton";
...
const ComponentCard = ({
  ...
  const hasMultiplePrices = (() => {
    ...
    if (result && component.name) {
      logger.info(`🏷️  Multiple prices found for ${component.name}:`, {
...
// PeripheralCard Component - displays individual peripherals
const PeripheralCard = ({
  ...
  logger.debug(`🖼️ PeripheralCard for ${peripheral.name}:`, {
...
export function PCBuilder({
  ...
  const { user } = useAuth();
  const [selectedComponents, dispatchSelections] = useReducer(
    selectionReducer,
    initialSelectionState
  );
  ...
  const setSelectedComponents = useCallback(
    (
      updater:
        ...
        | ((prev: SelectedComponentIds) => SelectedComponentIds)
    ) => {
      if (typeof updater === "function") {
        const next = (
          updater as (prev: SelectedComponentIds) => SelectedComponentIds
        )(selectedComponentsRef.current);
        dispatchSelections({ type: "SET_ALL", payload: next });
      } else {
        dispatchSelections({ type: "SET_ALL", payload: updater });
      }
    },
    []
  );
  ...
  // UI state consolidated in reducer
  ...
  // Stable setter adapters for backward compatibility
  const setViewMode = useCallback(
    (mode: "grid" | "list") => dispatchUI({ type: "SET_VIEW_MODE", mode }),
    []
  );
  const setSortBy = useCallback(
    (sortBy: string) => dispatchUI({ type: "SET_SORT_BY", sortBy }),
    []
  );
  const setCurrentPage = useCallback(
    (page: number) => dispatchUI({ type: "SET_CURRENT_PAGE", page }),
    []
  );
  const setCategoryPages = useCallback(
    (
      updater:
      ...
      if (typeof updater === "function") {
        const updated = updater(categoryPages);
        Object.entries(updated).forEach(([cat, page]) =>
          dispatchUI({ type: "SET_CATEGORY_PAGE", category: cat, page })
        );
      } else {
        Object.entries(updater).forEach(([cat, page]) =>
          dispatchUI({ type: "SET_CATEGORY_PAGE", category: cat, page })
        );
      }
    },
  ...
  const setSearchQuery = useCallback(
    (query: string) => dispatchUI({ type: "SET_SEARCH_QUERY", query }),
    []
  );
  const setGlobalSearchQuery = useCallback(
    (query: string) => dispatchUI({ type: "SET_GLOBAL_SEARCH", query }),
    []
  );
  const setSelectedBrands = useCallback(
    (brands: string[]) => dispatchUI({ type: "SET_BRANDS", brands }),
    []
  );
  const setPriceRange = useCallback(
    (range: [number, number]) => dispatchUI({ type: "SET_PRICE_RANGE", range }),
    []
  );
  const setOptionFilters = useCallback(
    (filters: Record<string, string[]>) =>
      dispatchUI({ type: "SET_OPTION_FILTERS", filters }),
    []
  );
  ...
  }, [activeCategory, categoryPages, setCurrentPage]);
  ...
  // searchQuery, globalSearchQuery, selectedBrands, priceRange, optionFilters now from uiState
    ...
    const loadCmsData = async () => {
      ...
      try {
        ...
        if (hasComponents || hasExtras) {
          logger.success("✅ Using CMS data for PC Builder");
        } else {
  ...
  }, [recommendedBuild, useCmsData, cmsComponents, setSelectedComponents]);
  ...
  useEffect(() => {
    ...
    setOptionFilters({});
    setRangeFilters({});
  }, [activeCategory, setSelectedBrands, setSearchQuery, setOptionFilters]);

  // Reset price range to current bounds
  useEffect(() => {
    setPriceRange([priceMin, priceMax]);
  }, [priceMin, priceMax, activeCategory, setPriceRange]);
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(async () => {
      try {
        ...
        logger.info("[PC Builder] Tracking category search", {
          query,
        ...
        if (!result.success) {
          ...
          logger.success("[PC Builder] Search tracked successfully");
        }
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(async () => {
      try {
        ...
        logger.info("[PC Builder] Tracking global search", {
          query,
          resultsCount,
        });
        ...
        if (!result.success) {
          ...
          logger.success("[PC Builder] Global search tracked successfully");
        }
                    ...
                    (() => {
                      ...
                      const getFPSTier = () => {
                        const gpu = selectedComponents.gpu
                          ? (activeComponentData as ComponentDataMap).gpu?.find(
                                    ...
                                    <Tooltip>
                                      ...
                                      <TooltipContent className="max-w-xs">
                                        <p className="text-xs">
                                          ...
                                          measures how well your components work
                                          together. A-grade = perfect harmony,
                                          F-grade = bottlenecks or compatibility
                                          issues.
                                        </p>
                                      </TooltipContent>
                                    </Tooltip>
