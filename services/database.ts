...
export interface Order {
  ...
  total: number;
  status: 'pending' | 'building' | 'testing' | 'shipped' | 'delivered' | 'completed';
  progress: number;
  orderDate: Date;
 ...
 */
export const getUserConfigurations = async (userId: string): Promise<SavedConfiguration[]> => {
  try {
    const q = query(
 ...
 */
export const trackPageView = async (userId: string | null, page: string): Promise<void> => {
  try {
