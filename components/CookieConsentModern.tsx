...
interface CookieConsentModernProps {
  onNavigate: (page: string) => void;
}

export function CookieConsentModern({ onNavigate }: CookieConsentModernProps) {
  const [showBanner, setShowBanner] = useState(false);

  useEffect(() => {
    // Clear any conflicting cookie consent keys
    const oldKeys = [
      "vortex_cookie_consent",
      "vortexpcs-cookie-consent",
      "vortexpcs-cookie-consent-blue",
    ];
    oldKeys.forEach((key) => localStorage.removeItem(key));

    const cookieConsent = localStorage.getItem(
      "vortexpcs-modern-cookie-consent"
    );
    if (!cookieConsent) {
              ...
              <div className="relative">
                ...
                <div className="relative w-12 h-12 bg-gradient-to-br from-sky-500/20 to-blue-500/20 border border-sky-500/40 rounded-full flex items-center justify-center">
                  ...
                  >
                    ...
                    <path
                      d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8 8-8 8 3.59 8 8 8s8-3.59 8-8-3.59 8-8 8z"
                      fill="currentColor"
                      opacity="0.6"
                    />
                  </svg>
