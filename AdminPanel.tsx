...
import type {
  ...
  RefundRequest as DBRefundRequest,
  RawUserRecord,
} from "../services/database";
...
export function AdminPanel() {
  ...
  // Add-step form state for build timeline
  const [stepProgress, setStepProgress] = useState<string>("");
  const [stepStatus, setStepStatus] = useState<Order["status"] | "">("");
  const [stepNote, setStepNote] = useState("");
  const [addingStep, setAddingStep] = useState(false);

  const [supportTickets, setSupportTickets] = useState<DBSupportTicket[]>([]);

  const [refundRequests, setRefundRequests] = useState<DBRefundRequest[]>([]);
  ...
  // Add a single build update step to the order's timeline without changing slider state
  const handleAddBuildStep = async () => {
    if (!selectedOrder?.id) return;
    ...
    const p = stepProgress.trim();
    const asNumber = p === "" ? NaN : Number(p);
    if (!Number.isNaN(asNumber))
      payload.progress = Math.max(0, Math.min(100, asNumber));
    if (stepStatus) payload.status = stepStatus as Order["status"];
    if (stepNote.trim()) payload.note = stepNote.trim();

    if (Object.keys(payload).length === 0) {
      alert("Please set progress, status, or note before adding a step.");
      return;
    }

    try {
      setAddingStep(true);
      await addBuildUpdate(selectedOrder.id, payload);

      // Refresh orders and keep selection in sync
      const orders = await getAllOrders(100);
      setAllOrders(orders);
      setRecentOrders(orders.slice(0, 5));
      const updated = orders.find((o) => o.id === selectedOrder.id) || null;
      ...
      setStepStatus("");
      setStepNote("");
      alert("Build step added.");
    } catch (err) {
      console.error("Add build step error", err);
      alert("Failed to add build step");
    } finally {
      setAddingStep(false);
    }
  };

  // Helper: normalize Firestore Timestamp | Date | number | string -> Date
  type BuildUpdateUI = {
    timestamp?: Date | { toDate: () => Date } | number | string | null;
    progress?: number;
    status?: string;
    note?: string;
  };
  const normalizeTimestamp = (t: BuildUpdateUI["timestamp"]): Date => {
              ...
              {supportTickets.length > 0 && (
                <Card className="bg-white/5 border-white/10 backdrop-blur-xl p-6">
                  ...
                  <div className="space-y-4">
                    {supportTickets
                      .filter(
                        (ticket): ticket is DBSupportTicket & { id: string } =>
                          !!ticket.id
                      )
                      .slice(0, 10)
                      .map((ticket) => (
              ...
              <div className="space-y-6 mt-6">
                ...
                {selectedOrder && (
                  <Card className="bg-white/5 border-white/10 p-4">
                    ...
                    {Array.isArray(selectedOrder.buildUpdates) &&
                    selectedOrder.buildUpdates.length > 0 ? (
                      <div className="space-y-3">
                        {(() => {
                          const updates =
                            (selectedOrder.buildUpdates as unknown as BuildUpdateUI[]) ||
                            [];
                          return updates
                            .slice()
                            .sort(
                            ...
                            .map((u, idx) => {
                              const d = normalizeTimestamp(u.timestamp);
                              return (
                                <div
                                  key={idx}
                                  className="flex items-start justify-between bg-white/5 border border-white/10 rounded p-3"
                                >
                                  <div className="flex-1">
                                    <div className="text-sm text-gray-400">
                                      {isNaN(d.getTime())
                ...
                <Card className="bg-white/5 border-white/10 p-4">
                  ...
                  <div className="grid md:grid-cols-3 gap-3">
                    <div>
                      ...
                      <Input
                        ...
                        max={100}
                        value={stepProgress}
                        onChange={(e) => setStepProgress(e.target.value)}
                    ...
                    <div>
                      ...
                      <select
                        value={stepStatus}
                        onChange={(e) =>
                          setStepStatus(e.target.value as Order["status"] | "")
                        }
                        className="w-full h-10 bg-white/5 border border-white/10 text-white rounded-md px-3"
                      >
                    ...
                    <div className="md:col-span-3">
                      ...
                      <Textarea
                        ...
                        placeholder="E.g., Installed GPU, began stress testing..."
                  ...
                  <div className="flex justify-end mt-3">
                    <Button
                      ...
                      disabled={addingStep || !selectedOrder}
                      className="bg-gradient-to-r from-sky-600 to-blue-600 hover:from-sky-500 hover:to-blue-500"
                    >
                      {addingStep ? (
                        <>
